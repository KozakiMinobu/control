    ...
    judgeBoating() {
      ...
      if (this.thunder === '雷鳴が聞こえる・雷注意報が出ている') {
        this.results = {
          ...
          crews: [
            { type: '漕歴 2 年以上の漕手がいない艇', status: 'prohibited', message: '乗艇禁止' },
            { type: '漕歴 2 年以上の漕手がクルーの半数以下の', status: 'prohibited', message: '乗艇禁止' },
            { type: '漕歴 2 年以上の漕手がクルーの半数より多い', status: 'prohibited', message: '乗艇禁止' }
          ]
        };
        return;
      }
      ...
      const crewTypes = [
        '漕歴 2 年以上の漕手がいない',
        '漕歴 2 年以上の漕手がクルーの半数以下の',
        '漕歴 2 年以上の漕手がクルーの半数より多い'
      ];
    ...
    getRiskLevel(crewType) {
      ...
      if (waveType === '波がない、もしくは微弱') {
        if (waterTempType === '15℃未満') {
          if (crewType === '漕歴 2 年以上の漕手がいない') {
            if (ws >= 0 && ws <= 2) return 'blue';
            if (ws >= 3 && ws <= 4) return 'yellow';
            return 'red'; // ws >= 5
          } else if (crewType === '漕歴 2 年以上の漕手がクルーの半数以下の') {
